...
def get_network_from_plans(
    ...
    if nw_class is None:
        ...
        if nw_class is not None:
            ...
            warnings.warn(
                f"Network class {network_class} not found in dynamic_network_architectures.architectures."
                f"Attempting to locait it within tmima..."
            )
            import tmima

            nw_class = recursive_find_python_class(
                join(tmima.__path__[0], "training"),
                network_class.split(".")[-1],
                "tmima.training",
            )
            if nw_class is not None:
